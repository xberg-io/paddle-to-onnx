...
class OPConvertAutoScanTest(unittest.TestCase):
    ...
    def run_and_statis(
        ...
        logging.info(f"Number of Generated Programs: {self.num_ran_models}")
        ...
        logging.info(f"Used time: {round(used_time, 2)} s")
        if max_duration > 0 and used_time > max_duration:
            logging.error(
                f"The duration exceeds {max_duration} seconds, if this is neccessary, try to set a larger number for parameter `max_duration`."
            )
            raise AssertionError()
