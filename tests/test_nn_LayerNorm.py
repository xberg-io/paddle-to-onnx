...
class Net(paddle.nn.Layer):
    ...
    def __init__(self):
        super().__init__()
        self._layer_norm = paddle.nn.LayerNorm(normalized_shape=[1, 10, 10])

    def forward(self, inputs):
        """
        forward
        """
        return self._layer_norm(inputs)


@_test_with_pir
def test_LayerNorm_9():
    """
    api: paddle.LayerNorm
    op version: 9
    """
    op = Net()
    op.eval()
    # net, name, ver_list, delta=1e-6, rtol=1e-5
    obj = APIOnnx(op, "nn_LayerNorm", [9])
    obj.set_input_data(
        "input_data",
...
def test_LayerNorm_10():
    ...
    op.eval()
    # net, name, ver_list, delta=1e-6, rtol=1e-5
    obj = APIOnnx(op, "nn_LayerNorm", [10])
    obj.set_input_data(
        "input_data",
        paddle.to_tensor(randtool("float", -1, 1, [3, 1, 10, 10]).astype("float32")),
    )
...
def test_LayerNorm_12():
    ...
    op.eval()
    # net, name, ver_list, delta=1e-6, rtol=1e-5
    obj = APIOnnx(op, "nn_LayerNorm", [12])
    obj.set_input_data(
        "input_data",
        paddle.to_tensor(randtool("float", -1, 1, [3, 1, 10, 10]).astype("float32")),
    )
