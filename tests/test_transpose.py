...
class Net(paddle.nn.Layer):
    ...
    def forward(self, inputs):
        ...
        forward
        """
        return paddle.transpose(inputs, perm=[1, 0, 2])
...
def test_transpose_9():
    ...
    op = Net()
    op.eval()
    obj = APIOnnx(op, "transpose", [9])
    obj.set_input_data(
        "input_data",
        paddle.to_tensor(randtool("float", -1, 1, [1, 3, 10]).astype("float32")),
    )
    obj.run()
