...
class Net(paddle.nn.Layer):
    ...
    def forward(self, inputs):
        ...
        forward
        """
        return paddle.nn.functional.leaky_relu(inputs)


@_test_with_pir
def test_leaky_relu_9():
    """
    api: paddle.leaky_relu
    op version: 9
    """
    op = Net()
    op.eval()
    # net, name, ver_list, delta=1e-6, rtol=1e-5
    obj = APIOnnx(op, "leaky_relu", [9])
    obj.set_input_data(
        "input_data",
