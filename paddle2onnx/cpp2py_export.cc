...
PYBIND11_MODULE(paddle2onnx_cpp2py_export, m) {
  ...
  m.def("export", [](const std::string &model_filename,
    ...
    int index = 0;
    for (auto &item : info) {
      const size_t op_name_size = sizeof(ops[index].op_name);
      const size_t export_op_name_size = sizeof(ops[index].export_op_name);
      snprintf(ops[index].op_name, op_name_size, "%s", item.first.c_str());
      snprintf(ops[index].export_op_name, export_op_name_size, "%s",
               item.second.c_str());
      index += 1;
    }
    char *out = nullptr;
  ...
  m.def("convert_to_fp16",
        [](const std::string &fp32_model_path,
           const std::string &fp16_model_path, bool verbose = false) {
