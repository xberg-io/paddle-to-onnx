...
bool ModelExporter::IsOpsRegistered(const PaddlePirParser &pir_parser,
  ...
  std::unordered_set<std::string> skip_set = {
      "pd_op.data",  "pd_op.feed", "pd_op.fetch", "pd_op.if",
      "pd_op.while", "cf.yield",   "pd_op.print"};
  for (auto op : pir_parser.total_blocks_ops) {
    if (skip_set.count(op->name()))
      continue;
  ...
  // ~keep TODO(wangmingkai02): detect whether unsupported ops are experimental.
...
int32_t
ModelExporter::GetCfBlockMinOpsetVersion(const PaddlePirParser &pir_parser,
...
void ModelExporter::ExportParameters(
  ...
  for (auto &item : parser.params) {
    // ~keep TODO(jiangjiajun): handle use_initializer.
...
ModelExporter::ExportIfBlock(const PaddlePirParser &pir_parser,
                             const pir::Block &block) {
  ...
  auto graph = std::move(ExportBlock(pir_parser, blockPtr, temp_parameters,
                                     &temp_inputs, &temp_outputs, true, false));
  pir_parser.sub_blocks_ops.clear();
  pir_parser.sub_blocks_ops = sub_blocks_ops_copy;
  return graph;
}
...
ONNX_NAMESPACE::GraphProto ModelExporter::ExportBlock(
    const PaddlePirParser &pir_parser, const pir::Block *block,
  ...
  for (auto i = 0; i < num_ops; ++i) {
    ...
    ExportOp(pir_parser, &temp_helper, opset_version_, op, i, if_in_subblock,
             verbose_);
  }
  if (if_in_subblock && !is_while_block) {
    for (auto &input_item : *inputs) {
      for (int32_t idx = 0; idx < outputs->size(); ++idx) {
        auto output_item = outputs->at(idx);
        if (output_item->name() == input_item->name()) {
          ...
          temp_helper.MakeNode("Identity", {input_item->name()},
                               {output_item->name()});
          outputs->at(idx) = std::move(output_item);
        }
      }
    }
    inputs->clear();
...
ONNX_NAMESPACE::GraphProto ModelExporter::ExportBlock(
  ...
  if (parser.is_quantized_model) {
    ...
    quantize_processer_->ProcessQuantizeModel(
        parameters, inputs, outputs, &(temp_helper->nodes), temp_helper, parser,
        calibration_cache_);
...
void ModelExporter::ExportOp(const PaddlePirParser &pir_parser,
                             OnnxHelper *helper, int32_t opset_version,
                             pir::Operation *op, int64_t op_id,
                             bool if_in_subblock, bool verbose) {
  P2OLogger(verbose_) << "Start export " << op->name()
                      << ", op_id: " << op->id() << std::endl;
  auto mapper = MapperHelper::Get()->CreateMapper(
      convert_pir_op_name(op->name()), pir_parser, helper, op_id,
      if_in_subblock);
...
void ModelExporter::ONNXChecker(const ONNX_NAMESPACE::ModelProto &model) {
  // ~keep TODO(jiangjiajun): define ONNX checker failure handling.
  try {
...
std::string ModelExporter::Run(
    const PaddlePirParser &pir_parser, int opset_version,
    bool auto_upgrade_opset, bool verbose, bool enable_onnx_checker,
    bool enable_experimental_op, bool enable_optimize,
    const std::string &deploy_backend, std::string *calibration_cache,
    const std::string &external_file, bool *save_external,
    bool export_fp16_model, std::vector<std::string> disable_fp16_op_types) {
  verbose_ = verbose;
  ...
  auto share_graph = ExportBlock(pir_parser, pir_parser.pir_program_->block(),
                                 parameters, &inputs, &outputs, false, false);
