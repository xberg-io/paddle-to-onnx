...
REGISTER_PIR_MAPPER(pad3d, Pad3DMapper)
...
void Pad3DMapper::Opset7() {
  ...
  if (data_format_ == "NDHWC") {
    input_name = helper_->Transpose(input_info[0].name, {0, 4, 1, 2, 3});
  }
  ...
  if (data_format_ == "NDHWC") {
    auto node = helper_->MakeNode("Pad", {input_name});
    ...
    auto node = helper_->MakeNode("Pad", {input_name}, {output_info[0].name});
    AddAttribute(node, "mode", mode);
    AddAttribute(node, "value", value_);
    AddAttribute(node, "pads", new_paddings);
  }
