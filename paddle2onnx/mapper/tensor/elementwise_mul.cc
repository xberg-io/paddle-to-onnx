...
#include "paddle2onnx/mapper/tensor/elementwise_mul.h"

namespace paddle2onnx {
REGISTER_MAPPER(elementwise_mul, ElementwiseMulMapper)

int32_t ElementwiseMulMapper::GetMinOpsetVersion(bool verbose) {
...
void ElementwiseMulMapper::ExportForONNX() {
  auto input_x_info = GetInput("X");
  auto input_y_info = GetInput("Y");
...
void ElementwiseMulMapper::ExportForRKNN() {
  auto input_x_info = GetInput("X");
  auto input_y_info = GetInput("Y");
  auto output_info = GetOutput("Out");
  ...
  std::string output_name;
  if (input_x_info[0].Rank() == input_y_info[0].Rank()) {
    output_name = helper_->MakeNode("Mul", {x_name, y_name})->output(0);
  } else {
    if (axis_ == -1) {
      axis_ = 1;
...
void ElementwiseMulMapper::Opset7() {
