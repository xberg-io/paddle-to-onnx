# A fake target to include all the libraries and tests the paddle2onnx module
# depends.
...
# Bundle several static libraries into one. This function is modified from
# Paddle Lite. reference:
# https://github.com/PaddlePaddle/Paddle-Lite/blob/develop/cmake/lite.cmake#L252
...
function(_recursively_collect_dependencies input_target)
    ...
    if((${_input_type} STREQUAL "INTERFACE_LIBRARY") OR (${_input_type} STREQUAL
    "STATIC_LIBRARY"))
        set(_input_link_libraries INTERFACE_LINK_LIBRARIES)
    endif()
        get_target_property(_public_dependencies ${input_target}
    ...
    foreach(dependency IN LISTS public_dependencies)
        if(TARGET ${dependency})
            ...
            list(FIND REDUNDANT_STATIC_LIBS ${dependency} _deps_redunant_id)
                if(${_type} STREQUAL "STATIC_LIBRARY" AND (NOT (${_deps_redunant_id}
        GREATER 0)))
...
list(REMOVE_ITEM static_libs ${REDUNDANT_STATIC_LIBS})
    message(
    STATUS "Found all needed static libs from dependecy tree: ${static_libs}")
    ...
    message(
    STATUS
      "Use bundled_tgt_full_name:  ${CMAKE_CURRENT_BINARY_DIR}/${CMAKE_STATIC_LIBRARY_PREFIX}${bundled_tgt_name}${CMAKE_STATIC_LIBRARY_SUFFIX}"
    )
...
if(WIN32)
    ...
    add_custom_target(
    ${fake_target} ALL COMMAND ${CMAKE_COMMAND} -E echo
    "Building fake_target ${fake_target}")
...
if(NOT IOS AND NOT APPLE)
    ...
    message(STATUS "Found ar_tool: ${ar_tool}")
      ...
      COMMAND ${CMAKE_STRIP} --strip-unneeded
              ${CMAKE_CURRENT_BINARY_DIR}/lib${bundled_tgt_name}.a
      COMMENT "Stripped unneeded symbols in ${bundled_tgt_name}" DEPENDS
              ${tgt_name}
        VERBATIM)
else()
